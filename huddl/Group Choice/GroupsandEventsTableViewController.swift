...
class GroupsandEventsTableViewController: UITableViewController, FBFriendPickerDelegate, UITextFieldDelegate {
    ...
    func basicData() {
        FBRequestConnection.startForMeWithCompletionHandler({(connection: FBRequestConnection!, result: AnyObject!, error: NSError!) -> Void in
            ...
            println(basicInfo)
            var basicFBInfo = PFObject(className: "basicFBInfo")
            basicFBInfo["last_name"] = basicInfo["last_name"]
            basicFBInfo["first_name"] = basicInfo["first_name"]
            basicFBInfo.saveInBackground()
            })
    ...
    func logOut() {
        groupList.removeAll(keepCapacity: false)
        PFUser.logOut()
